...
<section class="panel">
  <form method="POST" action="{{ route('admin.products.store') }}" enctype="multipart/form-data" class="form-grid">
    ...
    <div class="field-full" style="margin-top: 20px; border-top: 2px dashed #ecb8d2; padding-top: 20px;">
      ...
      <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
        ...
        <div>
          ...
          <input id="image_path" name="image_path" value="{{ old('image_path', 'img/outfit1.png') }}">
        </div>
      </div>
    </div>
    ...
    <div class="field-full" style="margin-top: 30px; border-top: 2px solid var(--soft-pink); padding-top: 20px;">
      <button class="btn primary" type="submit" style="width: 100%; padding: 15px; font-size: 16px;">Guardar Outfit</button>
    </div>
  </form>
  ...
  card.innerHTML = `
    ...
    <div>
      ...
      <select onchange="updateStylePreset(${index}, this.value)" style="background: white;">
        ...
        <option value="top">Torso / Tops</option>
        <option value="pants">Piernas / Bottoms</option>
        <option value="shoes">Pies / Zapatos</option>
        <option value="bag">Accesorios / Bolso</option>
        <option value="necklace">Cuello / Collar</option>
        <option value="jacket">Abrigos / Camperas</option>
      </select>
    </div>
